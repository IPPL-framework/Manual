...
Build Kokkos Tools against a toolchain and Kokkos version compatible with the IPPL executable being profiled. The current IPPL baseline is Kokkos 5.2.0, so profiling libraries should be built from a Kokkos Tools revision that supports the Kokkos 5.2 tools interface and the same backend stack (CUDA, HIP, OpenMP, or Serial) used by the application.
...
#include <Kokkos_Core.hpp>
  ...
  typedef Kokkos::View<int*, device_memory> a_type;
  typedef Kokkos::View<int*, Kokkos::HostSpace> b_type;
#ifdef KOKKOS_ENABLE_CUDA
  typedef Kokkos::View<int*, Kokkos::CudaHostPinnedSpace> c_type;
#else
  typedef Kokkos::View<int*, Kokkos::HostSpace> c_type;
#endif
...
This produces output similar to the following. Exact memory-space names depend on the configured Kokkos 5.2.0 backend and enabled memory spaces.
