...
As mentioned, IPPL provides the main building blocks for particle-mesh simulations. One such method is the Particle-in-Cell (PIC) method. In this section, we give a brief overview of the physics and numerical theory behind it. For further information, please see [@hockney1980] and [@birdsall2018]. 
...
\begin{align}
	 \frac{d\vec{x}}{dt} &= \vec{v}, \label{eq:motion_x} \\
	  \frac{d\vec{v}}{dt} &= \frac{q_s}{m_s}(\vec{E} + \vec{v}\cross\vec{B}) = \frac{\vec{F}}{m_s}, \label{eq:motion_v}
\end{align}
$$
...
$$
\begin{align}
	\vec{\nabla}\cross\vec{E} &= -\frac{\partial \vec{B}}{\partial t},  & \text{(Faraday's law)} \label{eq:maxwell_eq1} \\
	\vec{\nabla}\cross\vec{B} &= \mu_0 \vec{J} + \mu_0\epsilon_0 \frac{\partial \vec{E}}{\partial t}, & \text{(Ampère's law)}  \label{eq:maxwell_eq2}\\
